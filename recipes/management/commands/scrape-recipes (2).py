...
def recursive_parser(response, deepness):
    ...
    if title_recipe in titles:
        skipped += 1
        print("SKipped because of same title")
        return
        ...
        *_, random_recipe = soup.findAll('a', attrs={"class": "MRTN__sc-gkm9mr-3 jXEnlf"})
        try:
            ...
            try:
                ...
                print("Unknown error while fetching", href)
                return
            thread = Thread(target=recursive_parser, args=(response, deepness))
            thread.start()
            threads.append(thread)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not os.path.exists('pickle'):
            os.mkdir('pickle')
        dump(recipes_list, open("pickle/recipes2.pkl", 'wb'))
        dump(ingredients, open("pickle/ingredients2.pkl", 'wb'))
        dump(utensils, open("pickle/utensils2.pkl", 'wb'))

        print(f"{skipped} recipes are skipped")
